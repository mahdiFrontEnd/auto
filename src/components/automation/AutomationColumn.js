...
import ChangeStatusCorrespondence from './AutomationFunctionButtons/ChangeStatusCorrespondence';
import TreeSelectFilterTable from '../MicroComponents/table/TreeSelectFilterTable';
import ShowImageBox from '../MicroComponents/ShowImageBox';
import { isFactorArray, paymentTypes } from '../../helper/jsons/paymentTypes';
...
const AutomationColumn = (address, hiddenItem) => {
  ...
  const GetAutomationUser = () => {
    GetAutomationUserList(setUserList, null, { deactivate: 1 });
  };
  ...
  const columns = [
      ...
      render: (text, row) => {
        ...
        return (
          <div className="text-start d-flex align-items-center gap-2 ">
            <ShowImageBox handleClick={() => {
              funcHandleSetParams([{ key: 'from', value: [row.created_user_id] }]);
            }} showSearch searchTooltip="جستجوی این کاربر" name={text}
                          srcImage={row.image_of_created_by?.length > 4 ? row.image_of_created_by : '/noImage.png'} />
    ...
    ...(!hiddenItem.includes('paymentType')
      ? [
        {
          title: 'نوع پرداخت',
          ...
          ...getColumnSelectProps(
            ...
            null,
            (res) => {
              funcHandleSetParams([{ key: 'paymentType', value: res }]);
            }, 'multiple', false, {}, true,
            null, 'is_factor', 'فاکتور اصلی',
            isFactorArray, null, (res) => {
              funcHandleSetParams([{ key: 'is_factory', value: res }]);
            }
          ),
        },
      ]
          ...
          render: (x, c) => {
            return (
              <p className="mb-0">
                ...
                {c.before_total_estimate_time_month &&
                  <>
                    <hr className="my-1" />
                    <span className="text-muted">م ماه: {c.before_total_estimate_time_month}</span>
                    <br />
                    <span className="text-muted">م سال: {c.before_total_estimate_time_year}</span>
                  </>
                }
              </p>
            );
      ...
      ? [
        {
          ...
          fixed: 'right',
          render: (text, row) => {
            return (automationAddress === 'correspondence' && searchParams.get('status') === 'complete') ?
              <ChangeStatusCorrespondence rowData={row} /> : <AutomationFunctionButtons row={row} />;
