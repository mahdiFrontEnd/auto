...
const EditComment = ({ data }) => {
  ...
  const onFinish = (values) => {
    ...
    }).editComment(`automation_${automationAddress}`, data.id, values);
  };

  return (<>
      {loginUser.id === data.created_user_id && data.is_editable && (<div onClick={showModal}>

          <IconBtn TooltipText="ویرایش" btnClass="orangeIconBtn"
                   icon={<Edit2 variant="Bold" size={22} />} />
      ...
      <Modal
        width={700}
        open={open} title="ویرایش کامنت" onCancel={handleCancel}
        footer={[
