  ...
  return (
    <div className="d-flex flex-column align-items-center ">
      <div className="d-flex mb-5 w-100 justify-content-between  align-items-center ">
        <h3 className="fw-bold mb-0 text-white">رمز یکبار مصرف</h3>

        <Button size="sm" className="btn " color="gray" type="button" onClick={handleReturn}>

        <span className="d-flex gap-1  align-items-center">
           <FaArrowRight style={{ color:'white' }} size={22} />
          <span className="text-white">بازگشت</span>
        </span>
        </Button>
      </div>

      <h5 style={{ fontSize: '13px' }} className="mb-3 text-white">لطفا رمز یکبار مصرف ارسال شده را وارد
        کنید.</h5>
      <div dir="ltr" className="mb-4">
        <OtpInput inputType="tel" shouldAutoFocus name="code"
                  inputStyle={{ borderRadius: '5px', border: '1px solid #dee2e6', width: '50px', height: '50px' }}
                  value={otp}
                  onChange={(e) => {
                    setOtp(convertPersianNumberToEng(e));
                  }}
