...
import { Button, Checkbox, Form, Input, Radio, TimePicker, TreeSelect } from 'antd';
...
const CalendarForm = ({ data, handleCancel, isCreate }) => {
  ...
  useEffect(() => {
    GetAutomationUserList((x) => {
      ...
      setUserList([{
        'value': userJson?.id, 'title': 'خودم', disabled: true,
      }, ...x]);
      ...
      <div className="row">
        ...
        <div className="col-md-6">
          ...
          >
            ...
            <TreeSelect loading={userListLoading}
                        defaultValue={[`${userJson?.id}`]}
                        treeCheckable
                        showCheckedStrategy={SHOW_PARENT}
                        treeData={userList} treeNodeFilterProp="title"
                        onChange={(e) => {
