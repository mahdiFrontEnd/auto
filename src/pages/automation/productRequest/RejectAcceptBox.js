...
const RejectAcceptBox = ({ data }) => {
  ...
  useEffect(() => {
    ...
    if (other?.includes('buyer') && open) {
      ...
      }, setUserListLoading, { order: 'seller' }, setUserList);
    }
  }, [data, other, open]);
  useEffect(() => {
  ...
  return (<>
      {data?.next && <div className="text-center d-flex gap-1 d-flex justify-content-center">
        {
          data?.next?.map((item) => (
            <div key={item?.id} onClick={() => showModal(item)}>
              <IconBtn TooltipText={item?.status}
                       btnClass={`${item?.color}IconBtn`}
                       icon={<DynamicIcon iconName={item?.icon} size={22} />}
              /></div>
      ...
      <Modal
        ...
        footer={[
          ...
          <Button key="submit" className="defBtn orangeBtn px-4" loading={loading} onClick={() => {
            form.submit();
          }}>
            ثبت
          </Button>,
      ...
      >
        ...
        >
          ...
          {other?.includes('buyer') && open && <Form.Item
            ...
            rules={[
              {
                required: true,
                message: 'لطفا مسئول خرید را وارد کنید!',
              },
            ]}
