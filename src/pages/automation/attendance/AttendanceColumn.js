...
import TimeInBox from './TimeInBox';
...
const AttendanceColumn = () => {
  ...
  const GetAutomationUser = () => {
    GetAutomationUserList(setUserList
      //   (x) => {
      //   setUserList(
      //     x?.map((item) => ({
      //       value: item.id,
      ...
      render: (x, row) => {
        return (<div className="d-flex align-items-center justify-content-center gap-2">
            {(x && !row.tatil) ? <Tooltip title="تعجیل">
              <div className="text-danger d-flex gap-1 align-items-center"><FaPersonRunning size={20}
                                                                                            color="red" /><span>{row.tajil_time}</span>
              </div>
            </Tooltip> : '...'}
        ...
        return (<div className="d-flex flex-column gap-2">
            {
              row.automation_takhir ? (
                <Link target="_blank" rel="noopener noreferrer"
                      to={`/automation/request/request_detail/${row.automation_takhir?.id}`}>
            ...
            }
            {
              row.automation_tajil ? (
                <Link target="_blank" rel="noopener noreferrer"
                      to={`/automation/request/request_detail/${row.automation_tajil.id}`}>
                  <div className="d-flex align-items-center gap-2 text-black">
