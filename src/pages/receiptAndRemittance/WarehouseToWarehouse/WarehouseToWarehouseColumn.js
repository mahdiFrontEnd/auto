...
const WarehouseToWarehouseColumn = () => {
      ...
      render: (text, { next_step, next_status }) => {
        ...
        return (
          <div className="d-flex justify-content-center gap-2">
            {isMyRequest === 'true' ? (
              <Link
                to={`/receiptAndRemittance/warehouseToWarehouse/show/${text}?my_request=${isMyRequest}`}
              >
                <IconBtn TooltipText="نمایش" btnClass="blueIconBtn" icon={<Eye size={24} />} />
              </Link>
            ) : (
              <Link
                to={`/receiptAndRemittance/warehouseToWarehouse/${next_status ? getType(next_step[0]) : 'show'}/${text}?my_request=${isMyRequest}`}
              >
...
export default WarehouseToWarehouseColumn;
