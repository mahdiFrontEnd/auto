...
// import DeadTimeInputBox from './DeadTimeInputBox';
import ReasonInputBox from './ReasonInputBox';
...
const RowBox = ({ item, index }) => {
  ...
  useEffect(() => {
    form.setFieldsValue({
      ...
      // dead_time: item?.dead_time ? dayjs(item?.dead_time, 'YYYY-MM-DD HH:mm') : null,
    });
  }, [item]);
  ...
  return (<div>
    <Form
      ...
      <div className="d-flex align-items-center gap-2  mb-2 rounded-2 border px-2 py-2 ">
        {item.editable ? <ProductSelectBox handleChange={handleChange} /> : ''}
        {item.editable ? <AmountInputBox product={product} /> : ''}
        {/*{item.editable ? <DeadTimeInputBox item={item} index={index} /> : ''}*/}
        {item.editable ? <ReasonInputBox item={item} index={index} /> : ''}
        {/*{item.editable ? <ChoosePaceAndAmount index={index} form={form} /> : ''}*/}
