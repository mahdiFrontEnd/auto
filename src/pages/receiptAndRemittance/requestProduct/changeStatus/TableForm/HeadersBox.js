...
const HeadersBox = () => {
  const { buyerList, receiptAndRemittanceArray } = useSelector((state) => state.Storehouse);
  ...
  return (
    ...
    >
      ...
      {showJson?.newAmount ? <div className="flex-1 text-center">مقدار جدید</div> : ''}
      {buyerList.length ? <div className="flex-1 text-center">خریدار</div> : ''}
      {showJson?.Operation && <div className="flex-1 text-center" style={{ width: 'auto' }}>
        عملیات
      </div>}
      <div>نمایش</div>
    </div>
