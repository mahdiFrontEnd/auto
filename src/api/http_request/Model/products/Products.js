...
class Products extends HttpClient {
  ...
  createEditProductSuppliers(data, id) {
    return this.upload(`storehouse/supplier/${id ? 'update' : 'store'}/${id || ''}`, data);

  }

  ProductList() {
    return this.get(`process/form/list_product`);
  }

  ProductListStoreOut() {
    return this.get(`process/form/list_product_for_store_out`);
  }

  categoryList(apiAddress = 'list') {
    return this.get(`storehouse/category/${apiAddress}`);
  }
